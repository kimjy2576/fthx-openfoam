...
| D2 | **추출 j/f 가 상관식과 크게 다름** — CFD j=0.0202 f=0.1806 vs 상관식 j=0.0146 f=0.0171. f 가 10배는 정의 불일치 의심(단위셀 dp 는 상하류 연장 포함, 상관식 f 는 코어만). j 는 1.4배로 그럴듯. 미수렴(t≈100) 상태이기도 함 → 수렴 후 재측정 + f 정의 정렬 필요 | 열림 |
