...
| D2 | **추출 j/f 가 상관식과 크게 다름** — CFD j=0.0202 f=0.1806 vs 상관식 j=0.0146 f=0.0171. **1차 정렬 완료**: 코어 단면(x=44/88mm) sampledSurface 프로브 추가 → 코어만 ΔP 32.4 Pa (전체 56.8 의 57%), f 0.181→0.103. 상관식 0.0171 과 여전히 6배. j 는 1.4배. **아직 미수렴(t≈50 데이터)** 이므로 수렴 후 재측정이 우선. 그래도 차이가 남으면 (a) plain 상관식이 Re_Dc 1773·이 형상에서 부정확한 것이 실제 발견일 수 있음 — D 모드를 도입한 이유 자체 (b) A_o/A_c 정의 재확인 순으로 볼 것 | 열림 |
